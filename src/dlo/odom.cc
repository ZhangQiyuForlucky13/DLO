...
dlo::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {
  ...
  this->icp_sub = this->nh.subscribe(this->topic_lidar, 1, &dlo::OdomNode::icpCB, this);
  this->imu_sub = this->nh.subscribe(this->topic_imu, 1, &dlo::OdomNode::imuCB, this);
  ...
  this->keyframe_pgo_pub = this->nh.advertise<sensor_msgs::PointCloud2>("keyframe_pgo", 1, true);
  ...
  this->keyframe_pgo_cloud = pcl::PointCloud<PointType>::Ptr (new pcl::PointCloud<PointType>);
  ...
  cout << "open the file stream ! " << endl;
  ...
  string s2s = path.append("s2s.txt");
  string s2m = path.append("s2m.txt");


  ofss2s.open(s2s, ios::app);
  ofss2m.open(s2m, ios::app);
...
void dlo::OdomNode::getParams() {
  ...
  ros::param::param<std::string>("~topic_lidar", this->topic_lidar, "topic_lidar");
  ros::param::param<std::string>("~topic_imu", this->topic_imu, "topic_imu");
  // nh_global.param("topic_lidar",this->topic_lidar ,"");
  // nh_global.param("topic_imu",this->topic_imu,"");
  std::cout << this->topic_lidar << "  and  " << this->topic_imu << std::endl;

  // nh.param("topic_lidar",this->topic_lidar ,"");
  // nh.param("topic_imu",this->topic_imu,"");
  // std::cout << this->topic_lidar << "  and  " << this->topic_imu << std::endl;
...
void dlo::OdomNode::publishKeyframe() {
  ...
  if (this->keyframe_cloud->points.size() == this->keyframe_cloud->width * this->keyframe_cloud->height) {
    ...
    sensor_msgs::PointCloud2 keyframe_pgo_cloud_ros;
    pcl::toROSMsg(*this->keyframe_cloud, keyframe_cloud_ros);
    pcl::toROSMsg(*this->keyframe_pgo_cloud, keyframe_pgo_cloud_ros);
    keyframe_cloud_ros.header.stamp = this->scan_stamp;
    keyframe_cloud_ros.header.stamp = this->scan_stamp;
    keyframe_pgo_cloud_ros.header.frame_id = this->odom_frame;
    keyframe_pgo_cloud_ros.header.frame_id = this->odom_frame;
    this->keyframe_pub.publish(keyframe_cloud_ros);
    this->keyframe_pgo_pub.publish(keyframe_pgo_cloud_ros);
  }
...
void dlo::OdomNode::icpCB(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  // Preprocess points 预处理去除NAN点
  this->preprocessPoints();

  // Compute Metrics
  //std::thread metrics_thread;线程
  ...
  // Set initial frame as target  gicps2s的初始化
  ...
  //printf("setInputSources cost  %f ms \n", this->tictoc.toc());
  //ofss2s << this->tictoc.toc() << endl ;
  
  //this->setInputSources_gicps2m();
...
void dlo::OdomNode::getNextPose() {
  ...
  cout << "write s2s time to file !" << endl;
  ofss2s << this->tictoc.toc() << endl ;

  //printf("gicps2s cost  %f ms \n", this->tictoc.toc());
  ...
  cout << "write s2m time to file !" << endl;
  ofss2m << this->tictoc.toc() << endl ;
  //printf("gicp cost  %f ms \n", this->tictoc.toc());
